// @file server/database/repository/user-repository.js
...
export default class userRepository {
  async FetchUsers({ filters, page, limit, sort, excludeFields }) {
    try {
      const paginatedResults = await paginateAndFilter(userModel, {
        filters,
        page,
        limit,
        sort,
  ...
  async FindUserById(userId) {
    try {
      const user = await userModel.findOne({ userId }).select(["-password", "-salt"]);
      return user;
    } catch (err) {
      throw new APIError(
        "API Error",
  ...
  async getNotifPrefs(userId) {
    const user = await userModel.findOne({ userId }).select("notificationPrefs");
  ...
  async updateNotifPrefs(userId, prefs) {
    const user = await userModel.findOneAndUpdate(
      { userId },
      { $set: { "notificationPrefs.productUpdates": prefs.productUpdates,
                 "notificationPrefs.securityAlerts": prefs.securityAlerts,
                 "notificationPrefs.marketing":      prefs.marketing,
                 "notificationPrefs.weeklyDigest":   prefs.weeklyDigest } },
      { new: true }
